...
async function getGiftTreasureGame(chestId, newImageSrc) {
    ...
    const gift = {
        name: "+ шаг",
        about: "Вы получили дополнительный шаг!"
    };

    completeMockGame("game_1", gift);
...
document.getElementById('chest1').addEventListener('click', function() {
    if (!chestOpened) {
        getGiftTreasureGame(
            'chest1',
            '../static/images/miniGameImages/openChest1.png'
        );
    }
});
...
document.getElementById('chest2').addEventListener('click', function() {
    if (!chestOpened) {
        getGiftTreasureGame(
            'chest2',
            '../static/images/miniGameImages/openChest2.png'
        );
    }
});
...
document.getElementById('chest3').addEventListener('click', function() {
    if (!chestOpened) {
        getGiftTreasureGame(
            'chest3',
            '../static/images/miniGameImages/openChest3.png'
        );
    }
});